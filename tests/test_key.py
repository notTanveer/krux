...
def test_init_fail_unknown_policy_type(mocker, m5stickv, tdata):
    ...
    # Valid types are 0, 1, 2, 3 (TYPE_SINGLESIG, TYPE_MULTISIG, TYPE_MINISCRIPT, TYPE_SILENT_PAYMENT)
    # 4 is unknown
...
def test_init_silent_payment(mocker, m5stickv, tdata):
    ...
    assert key.scan_privkey is not None
    assert key.spend_pubkey is not None
